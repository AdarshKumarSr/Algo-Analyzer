...
function ExecutionStats({ execution }) {
    ...
    return (
        <div className={`flex flex-wrap gap-3 p-4 rounded-xl border mb-6 ${success ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
            <div className="flex items-center gap-2">
                <span className={`w-2 h-2 rounded-full ${success ? 'bg-green-500' : 'bg-red-500'}`} />
                <span className={`text-sm font-medium ${success ? 'text-green-700' : 'text-red-700'}`}>{execution.status}</span>
            </div>
            {execution.output && (
                <div className="flex items-center gap-2">
                    <span className="text-xs text-zinc-400">Output</span>
                    <code className="text-xs bg-white border border-zinc-200 px-2 py-0.5 rounded text-zinc-700">{execution.output}</code>
                </div>
            )}
            {execution.execution_time_ms && (
                <div className="flex items-center gap-2">
                    <span className="text-xs text-zinc-400">Time</span>
                    <span className="text-xs font-mono font-medium text-zinc-700">{execution.execution_time_ms.toFixed(2)} ms</span>
                </div>
            )}
            {execution.memory_kb && (
                <div className="flex items-center gap-2">
                    <span className="text-xs text-zinc-400">Memory</span>
                    <span className="text-xs font-mono font-medium text-zinc-700">{(execution.memory_kb / 1024).toFixed(1)} MB</span>
                </div>
            )}
            {execution.stderr && (
    ...
    return (
        <div className="border border-zinc-200 rounded-xl overflow-hidden">
            <div className="px-4 py-3 bg-zinc-50 border-b border-zinc-200">
                <p className="text-sm font-medium text-zinc-800">{suggestion.title}</p>
            </div>
            <div className="px-4 py-3">
                <p className="text-sm text-zinc-500 mb-3">{suggestion.description}</p>
                {suggestion.code && (
                    <div className="relative">
                        <pre className="text-xs bg-zinc-50 text-zinc-800 border border-zinc-200 p-3 rounded-lg overflow-x-auto font-mono">{suggestion.code}</pre>
                        <button onClick={copy} className="absolute top-2 right-2 text-xs px-2 py-1 rounded bg-white border border-zinc-200 text-zinc-500 hover:text-zinc-800 transition-colors">
                            {copied ? '✓' : 'Copy'}
                        </button>
                    </div>
                )}
    ...
    return (
        <div className="border border-violet-200 rounded-xl overflow-hidden">
            <div className="px-4 py-3 bg-violet-50 border-b border-violet-200 flex items-center justify-between">
                <div>
                    <p className="text-sm font-medium text-violet-800">Optimized version</p>
                    <p className="text-xs text-violet-400 mt-0.5">AI-generated improvement</p>
                </div>
            ...
            {show && (
                <div className="relative">
                    <pre className="text-xs bg-zinc-50 text-zinc-800 border-t border-zinc-200 p-4 overflow-x-auto font-mono leading-relaxed">{code}</pre>
                    <button onClick={copy} className="absolute top-3 right-3 text-xs px-2 py-1 rounded bg-white border border-zinc-200 text-zinc-500 hover:text-zinc-800 transition-colors">
                        {copied ? '✓' : 'Copy'}
                    </button>
                </div>
            )}
    ...
    return (
        <div className="border border-red-200 rounded-xl overflow-hidden">
            <div className="px-4 py-3 bg-red-50 border-b border-red-200">
                <p className="text-sm font-medium text-red-700">Bug found</p>
            </div>
            <div className="px-4 py-3 space-y-2">
                <div>
                    <p className="text-xs text-zinc-400 mb-1">Line</p>
                    <code className="text-xs bg-zinc-100 px-2 py-1 rounded font-mono text-zinc-700">{bug.line}</code>
                </div>
                <div>
                    <p className="text-xs text-zinc-400 mb-1">Issue</p>
                    <p className="text-sm text-zinc-700">{bug.issue}</p>
                </div>
                {bug.fix && bug.fix !== 'N/A' && (
                    <div>
                        <p className="text-xs text-zinc-400 mb-1">Fix</p>
                        <code className="text-xs bg-green-50 border border-green-200 px-2 py-1 rounded font-mono text-green-700">{bug.fix}</code>
                    </div>
                )}
            </div>
        </div>
        ...
        <div className="flex h-screen overflow-hidden bg-white">
            ...
            <main className="flex-1 overflow-y-auto p-6 pt-20">
                <div className="max-w-3xl mx-auto">
                    ...
                    <div className="mb-8 flex items-start justify-between flex-wrap gap-4">
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                <h1 className="text-2xl font-bold text-gray-800">Code Analyzer</h1>
                                <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-violet-100 text-violet-600 border border-violet-200">v2</span>
                            </div>
                            <p className="text-sm text-gray-400">Write your algorithm — we'll run it, measure it, and tell you exactly how to improve it</p>
                        </div>
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2.5 px-4 py-2 rounded-xl border text-sm font-medium transition-all ${
                                competitive
                                    ...
                                    : 'bg-white border-zinc-200 text-zinc-500 hover:border-violet-300 hover:text-violet-600'
                            }`}
                        >
                    ...
                    {competitive && (
                        <div className="mb-5 p-3 bg-violet-50 border border-violet-200 rounded-xl flex items-center gap-3">
                            <span className="text-sm">⚔️</span>
                            <p className="text-sm text-violet-700">Competitive mode on — suggestions and optimized code are hidden. You get one hint.</p>
                        </div>
                    )}
                        ...
                        <div className="flex gap-3 mb-8 items-end">
                            <div className="flex-1">
                                ...
                                <input
                                    ...
                                    className="w-full px-4 py-2.5 rounded-lg border border-zinc-200 bg-zinc-50 text-sm text-gray-700 outline-none focus:border-violet-300 focus:bg-white transition-colors"
                                />
                            </div>
                            <button
                    ...
                    {result && (
                        <div className="flex gap-3 mb-6">
                            <button onClick={() => { setResult(null); setError('') }} className="px-4 py-2 rounded-lg border border-zinc-200 text-sm text-zinc-500 hover:border-zinc-300 hover:text-zinc-700 transition-colors">
                                ← New code
                            </button>
                            <button onClick={handleAnalyze} disabled={loading} className="px-4 py-2 rounded-lg bg-violet-50 border border-violet-200 text-sm text-violet-600 hover:bg-violet-100 transition-colors">
                                ↺ Re-run
                            </button>
                        </div>
                    )}
                    ...
                    {/* Error */}
                    {error && (
                        <p className="text-red-500 text-sm mb-6 p-3 bg-red-50 border border-red-200 rounded-lg">{error}</p>
                    )}
                    ...
                    {loading && (
                        <div className="space-y-3 mb-8">
                            <div className="flex items-center gap-3 p-4 bg-zinc-50 rounded-xl border border-zinc-200">
                                ...
                                <div>
                                    <p className="text-sm text-zinc-600 font-medium">Running your code...</p>
                    ...
                    {result && !loading && (
                        <>
                            ...
                            <div className="flex items-center justify-between mb-4">
                                <h2 className="text-lg font-bold text-gray-800">{result.algorithm}</h2>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
                                {result.time_complexity?.explanation && (
                                    <div className="p-3 bg-zinc-50 rounded-lg border border-zinc-200">
                                        ...
                                        <p className="text-sm text-zinc-600">{result.time_complexity.explanation}</p>
                                    </div>
                                )}
                                {result.space_complexity?.explanation && (
                                    <div className="p-3 bg-zinc-50 rounded-lg border border-zinc-200">
                                        ...
                                        <p className="text-sm text-zinc-600">{result.space_complexity.explanation}</p>
                                    </div>
                                )}
                            </div>
                            ...
                            {competitive && result.competitive_hint && (
                                <div className="mb-6 p-4 bg-violet-50 border border-violet-200 rounded-xl">
                                    <p className="text-xs font-medium text-violet-500 uppercase tracking-wide mb-2">Your hint</p>
                                    <p className="text-sm text-violet-800">{result.competitive_hint}</p>
                                </div>
                            )}
                            ...
                            {competitive && (
                                <button
                                    ...
                                    className="w-full py-3 rounded-xl border-2 border-dashed border-zinc-200 text-sm text-zinc-400 hover:border-violet-300 hover:text-violet-500 transition-colors mb-6"
                                >
