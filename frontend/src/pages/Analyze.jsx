...
const MATCH_COLOR = (score) => {
  if (score >= 90) return { bar: 'bg-green-500',  text: 'text-green-600',  label: 'Logically identical' }
  if (score >= 70) return { bar: 'bg-green-400',  text: 'text-green-500',  label: 'Correct, minor differences' }
  if (score >= 50) return { bar: 'bg-amber-500',  text: 'text-amber-600',  label: 'Correct, missing edge cases' }
  if (score >= 30) return { bar: 'bg-orange-500', text: 'text-orange-600', label: 'Partially correct' }
  return              { bar: 'bg-red-500',    text: 'text-red-600',    label: 'Significant issues' }
}
    ...
    <div className="flex h-screen overflow-hidden bg-white">
      ...
      <main className="flex-1 overflow-y-auto p-6 pt-20">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-gray-800 mb-1">AI Code Analyzer</h1>
            <p className="text-sm text-gray-400">Paste your algorithm — we'll detect it, compare it to the reference, and visualize it</p>
          </div>
          ...
          {result && (
            <div className="flex gap-3 mb-6">
              <button
                ...
                className="px-4 py-2 rounded-lg border border-zinc-200 text-sm text-zinc-500 hover:border-zinc-300 hover:text-zinc-700 transition-colors"
              >
                ← Analyze new code
              </button>
              <button
                onClick={handleAnalyze}
                disabled={loading}
                className="px-4 py-2 rounded-lg bg-violet-50 border border-violet-200 text-sm text-violet-600 hover:bg-violet-100 transition-colors"
              >
                ↺ Re-analyze
              </button>
            </div>
          ...
          {result && !loading && (
            <>
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h2 className="text-lg font-bold text-gray-800">{result.name || result.slug}</h2>
